...
specify_texture <- function(colour = "black", background = "white", crop = FALSE, seed = 1) {
  ...
  modify <- function(data, x_shift, y_shift, scale, ind) {
  ...
  for(x_shift in seq(from = x_limit[1], to = x_limit[2], by = scale * 1.2)) {
    for(y_shift in seq(from = y_limit[1], to = y_limit[2], by = scale)) {
      ind <- ind + 1
      if(runif(1) < p) trapezoid_list[[ind]] <- modify(trapezoid, x_shift, y_shift, scale, ind)
      #if(runif(1) < p) slash_list[[ind]] <- modify(slash, x_shift, y_shift, scale, ind)
      if(runif(1) < p) bracket_list[[ind]] <- modify(bracket, x_shift, y_shift, scale, ind)
    }
  }
