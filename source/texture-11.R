...
generate_logotype <- function() {
  ...
  tibble(
    ...
    size = c(20, 10, 12, 10),
    hjust = c("left", "left", "middle", "middle"),
    vjust = c("center", "top", "center", "center")
  )
} 
...
specify_texture <- function(colour = "black", background = "white", crop = FALSE, seed = 1) {
  ...
  pic <- ggplot() +
    ...
    geom_polygon(
      data = base_trapezoid,
      mapping = aes(x, y + ysh, group = id, subgroup = part),
      fill = colour_code,
      colour = colour_code,
      size = .5
    ) +
