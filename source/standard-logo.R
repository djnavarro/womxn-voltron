...
make_standard_logo <- function(colour = "black", background = "white") {
...
for(r in 1:nrow(visual_themes)) {
  ...
  dir <- here::here("img/standard-logo")
  fname <- paste0("standard-logo_", col, "-on-", bg, ".png")
  cat(fname, "\n")
  
  pic <- make_standard_logo(colour = col, background = bg)
