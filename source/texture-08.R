...
build_grid <- function(x_limit, y_limit, scale = .1, base_prob = .4, frequency = .3) {
...
specify_texture <- function(colour = "black", background = "white", crop = FALSE, seed = 1) {
  ...
  for(i in 1:nrow(gr)) {
    if(runif(1) < gr$z[i]) trapezoid_list[[i]] <- modify(trapezoid, gr$x[i], gr$y[i], gr$scale[i], gr$prob[i], i)
    if(runif(1) < gr$z[i]) bracket_list[[i]] <- modify(bracket, gr$x[i], gr$y[i], gr$scale[i], gr$prob[i], i)
  }
