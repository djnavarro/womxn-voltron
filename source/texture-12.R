...
generate_logotype <- function() {
  ...
  tibble(
    ...
    font = c("archerus"),
    weight = c("bold", "italic", "italic", "italic"),  # ... these are lies!
    size = c(20, 10, 10, 10),
    hjust = c("left", "left", "right", "right"),
    vjust = c("center", "top", "top", "center")
  )
} 
...
build_grid <- function(x_limit, y_limit, scale = .1, base_prob = .4, frequency = .5) {
...
specify_texture <- function(colour = "black", background = "white", crop = FALSE, seed = 1) {
  ...
  modify <- function(data, x_shift, y_shift, scale, alpha, ind) {
    ...
    # special rules inside the strip enclosing the logo
    #if(x_shift < 8 & x_shift > -1) {
      if(y_shift > 2) {
        
        # drastically reduce the probability of inclusion
        if(runif(1) < 1) return(NULL)
  ...
  for(i in 1:nrow(gr)) {
    if(runif(1) < gr$z[i]) trapezoid_list[[i]] <- modify(trapezoid, gr$x[i], gr$y[i], gr$scale[i], gr$prob[i]*gr$alpha[i], i)
    if(runif(1) < gr$z[i]) bracket_list[[i]] <- modify(bracket, gr$x[i], gr$y[i], gr$scale[i], gr$prob[i]*gr$alpha[i], i)
  }
  ...
  pic <- ggplot() +
    ...
    geom_polygon(
      data = base_trapezoid,
      mapping = aes(x + xsh, y + ysh, group = id, subgroup = part),
      fill = colour_code,
      colour = colour_code,
      size = .5
    ) +
    geom_polygon(
      data = base_slash,
      mapping = aes(x + xsh, y + ysh, group = id),
      fill = colour_code,
      colour = colour_code,
      size = .1
    ) +
