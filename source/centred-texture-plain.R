...
make_centred_texture_plain <- function(colour, background, seed = 1000) {
  ...
  # define the texture
  blank_fn <- function(x, y) y < 2.25 & y > -1.25
  ...
  voltron_text <- generate_logotype(position = "", message = "") 
...
for(r in 1:nrow(visual_themes)) {
  ...
  dir <- here::here("img/centre-texture-plain")
  fname <- paste0("centre-texture-plain_", col, "-on-", bg, ".png")
  cat(fname, "\n")
  
  pic <- make_centred_texture_plain(colour = col, background = bg)
