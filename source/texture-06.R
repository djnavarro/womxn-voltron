...
build_grid <- function(x_limit, y_limit, scale = .2, base_prob = .3, frequency = .2) {
...
specify_texture <- function(colour = "black", background = "white", crop = FALSE, seed = 1) {
  ...
  for(i in 1:nrow(gr)) {
    if(runif(1) < gr$prob[i]) trapezoid_list[[i]] <- modify(trapezoid, gr$x[i], gr$y[i], gr$scale[i], gr$alpha[i], i)
    if(runif(1) < gr$prob[i]) bracket_list[[i]] <- modify(bracket, gr$x[i], gr$y[i], gr$scale[i], gr$alpha[i], i)
  }
